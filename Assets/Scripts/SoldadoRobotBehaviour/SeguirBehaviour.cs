...
public class SeguirBehaviour : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (Vector2.Distance(animator.transform.position, posicionJugador.position) > distanciaParar && Vector2.Distance(animator.transform.position, posicionJugador.position) < distanciaSeguir) // si la distancia entre el enemigo y el jugador es mayor a la distancia de parar y menor a la de seguir
        {           
            animator.transform.position = Vector2.MoveTowards(animator.transform.position, posicionJugador.position, velocidad * Time.deltaTime); // el enemigo se mueve en direccion del jugador
        ...
        if(Vector2.Distance(animator.transform.position, posicionJugador.position) < distanciaParar && Vector2.Distance(animator.transform.position, posicionJugador.position) > distanciaRetirar) // si la distancia entre el enemigo y el jugador es menor a la de parar y mayor a la de retirar
        {
            animator.SetBool("Disparar", true);
            animator.SetBool("Seguir", false);
        }else 
        if(Vector2.Distance(animator.transform.position, posicionJugador.position) < distanciaRetirar) // si la distancia entre el enemigo y el jugador es menor a la distancia de retirar
        {
            animator.SetBool("Retirar", true);
            animator.SetBool("Seguir", false);
        }else 
        if(Vector2.Distance(animator.transform.position, posicionJugador.position) > distanciaSeguir) // si la distancia entre el enemigo y el jugador es mayor a la de seguir
        {
            animator.SetBool("Repatrullar", true);
            animator.SetBool("Seguir", false);
        }
    }
