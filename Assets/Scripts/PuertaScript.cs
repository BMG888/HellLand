...
public class PuertaScript : MonoBehaviour
{
    ...
    public int linkPuertaId; // id para linkear la puerta con el trigger

    // Start is called before the first frame update
    void Start()
    {
        EventoManagerScript.instanciar.abrirPuertaEvento += Abrir; // se suscribe la puerta al evento
    }
    ...
    {
        if(linkPortonId == linkPuertaId) // si el link del trigger es igual al link de la puerta
        {            
    ...
    {
        EventoManagerScript.instanciar.abrirPuertaEvento -= Abrir; // de desuscribe al evento
    }
}
